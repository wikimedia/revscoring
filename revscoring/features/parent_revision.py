...
was_same_user = Feature("parent_revision.was_same_user",
                        process_was_same_user, returns=bool,
...
seconds_since = Feature("parent_revision.seconds_since",
                        process_seconds_since, returns=int,
                        depends_on=[parent_revision.metadata,
                                    revision.metadata])
...
def process_markup_chars(parent_revision_text):
    ...
    return sum(len(m.group(0)) for m in
               MARKUP_RE.finditer(parent_revision_text))
...
def process_numeric_chars(parent_revision_text):
    parent_revision_text = parent_revision_text or ""
    return sum(len(m.group(0)) for m in
               NUMERIC_RE.finditer(parent_revision_text))
...
def process_symbolic_chars(parent_revision_text):
    parent_revision_text = parent_revision_text or ""
    return sum(len(m.group(0)) for m in
               SYMBOLIC_RE.finditer(parent_revision_text))
