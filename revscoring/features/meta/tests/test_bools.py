...
def test_item_in_set():
    ...
    eq_(solve(is_a_sysop, cache={my_set: {'foo', 'bar'}}),
        False)
    eq_(solve(is_a_sysop, cache={my_set: {'foo', 'sysop'}}),
        True)
...
def test_set_contains_item():
    ...
    eq_(solve(is_me, cache={my_item: 999}),
        False)
    eq_(solve(is_me, cache={my_item: 6877667}),
        True)
...
def test_sets_intersect():
    has_small_odd = bools.sets_intersect({1, 2, 3, 5, 7, 11, 13}, my_set)

    eq_(solve(has_small_odd, cache={my_set: {4, 18, 10}}),
        False)
