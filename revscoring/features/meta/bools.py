...
class item_in_set(Feature):
    def __init__(self, item, set_datasource, name=None):
        self.item = item
        name = self._format_name(name, [item, set_datasource])
        super().__init__(name, self.process, depends_on=[set_datasource],
                         returns=bool)
