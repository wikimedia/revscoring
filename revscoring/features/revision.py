...
infonoise = Feature("revision.infonoise", process_infonoise, returns=float,
                    depends_on=[is_stopword, stem_word,
...
proportion_of_templated_references = cite_templates / \
    modifiers.max(ref_tags, 1)


def process_infobox_templates(revision_templates):
    return sum(1 for t in revision_templates if
               INFOBOX_RE.search(str(t.name)))

infobox_templates = Feature("revision.infobox_templates",
                            process_infobox_templates, returns=int,
                            depends_on=[revision.templates])
